...
- **#20 (병합 부분 해소, 투명도 잔여)**: 한 줄 연속 단어 병합 ✅ — `find_words_in_rect`이 이제 `(block_no, line_no)` + 연속 `word_no`로 그룹화하여 단일 merged rect 반환. 시각적 V-notch 제거됨. **잔여**: alpha 슬라이더 (Preferences > Colors)로 사용자 투명도 지정.
